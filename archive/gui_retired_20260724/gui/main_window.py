...
def _md_to_html(text: str) -> str:
    """Convert markdown to basic HTML for QTextEdit display."""
    lines = text.split('\n')
    html_lines = []
    in_list = False
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.tabs.addTab(self.create_chat_tab(), "💬 AI Chat")
    ...
    def create_chat_tab(self) -> QWidget:
        ...
        right = QVBoxLayout()
        self.chat_send_btn = QPushButton("🧠 Send to Grok")
        self.chat_send_btn.clicked.connect(self.send_chat)
        self.chat_send_btn.setToolTip("Ask Grok (xAI) for market analysis")
        right.addWidget(self.chat_send_btn)

        self.claude_chat_btn = QPushButton("🤖 Send to Claude")
        self.claude_chat_btn.clicked.connect(self.send_claude_chat)
        self.claude_chat_btn.setToolTip("Ask Claude directly with full portfolio context")
        right.addWidget(self.claude_chat_btn)
    ...
    def send_chat(self):
        ...
        self.chat_status_label.setText("Asking Grok...")
        ...
        def worker():
            try:
                ...
                if not api_key:
                    self.chat_signals.finished.emit({"error": "No Grok API key in config.yaml"})
                    return
                resp = _req.post(
                ...
                self.chat_signals.finished.emit({"answer": answer, "_source": "grok"})
            except Exception as e:
                self.chat_signals.finished.emit({"error": str(e)})

        threading.Thread(target=worker, daemon=True).start()

    def send_claude_chat(self):
        """Send chat message directly to Claude with portfolio context"""
        query = self.chat_input.toPlainText().strip()
        if not query:
            return
        self.chat_input.clear()
        ...
        self.chat_send_btn.setEnabled(False)
        self.claude_chat_btn.setEnabled(False)
    ...
    def _on_chat_response(self, resp: dict):
        ...
        self.claude_chat_btn.setEnabled(True)
        if resp.get("error"):
...
First provide your analysis, then end with the JSON block."""
                ...
                # Try code-fenced JSON first (handles nested braces properly)
                json_match = re.search(r'```json\s*(\{.*?\})\s*```', response_text, re.DOTALL)
                if not json_match:
                    # Try greedy match for nested braces
                    json_match = re.search(r'```json\s*(\{.*\})\s*```', response_text, re.DOTALL)
                if not json_match:
                    # Try without code blocks — find JSON with "stocks" key
                    json_match = re.search(r'(\{[^{}]*"stocks"\s*:\s*\[.*?\].*?\})', response_text, re.DOTALL)
                ...
                if json_match:
                    ...
                    try:
                        ...
                        try:
                            ...
                            logger.error(f"Failed to parse Claude ticker JSON: {raw_json[:200]}")
...
✅ **Changes saved.** Bot will use new tickers on the next trading cycle."""
                        ...
                        logger.error(f"Error updating tickers from Claude: {e}")
                        pass
    ...
    def _on_claude_response(self, resp: dict):
        ...
        if resp.get("error"):
            ...
            if resp.get("_tickers_updated"):
                self.chat_history.append(f"<b>System:</b><br>{formatted}")
                # Reload settings tab to show new tickers
                self.load_settings_config()
            elif resp.get("_claude_recommendation"):
                self.chat_history.append(f"<b>Claude:</b><br>{formatted}")
            else:
                self.chat_history.append(f"<b>Claude Analysis:</b><br>{formatted}")
