...
try:
    ...
    DexterGate = None
try:
    from insider_congress import get_insider_scores
...
try:
    ...
    talib = None
    # Defer warning until after logging is configured (avoid premature root logger init)
    _talib_missing = True
else:
    _talib_missing = False
import tweepy
...
# Only configure root logging if running standalone (not imported by bot_service)
...
if _talib_missing:
    logger.warning("TA-Lib is not installed. Technical indicators will not work.")

ALPACA_AUTH_STATUS_CODES = {401, 403}
ALPACA_TRANSIENT_STATUS_CODES = {408, 429, 500, 502, 503, 504}


def alpaca_status_code(exc: Exception) -> Optional[int]:
    """Best-effort extraction of HTTP status code from Alpaca/requests exceptions."""
    if exc is None:
        return None
...
def is_alpaca_auth_error(exc: Exception) -> bool:
    """Return True for authentication/authorization failures."""
    status_code = alpaca_status_code(exc)
...
def is_transient_alpaca_error(exc: Exception) -> bool:
    """Return True for network/transient API errors that are usually worth retrying."""
    if isinstance(
        exc,
        (
            ConnectionError,
    ...
    status_code = alpaca_status_code(exc)
    if status_code in ALPACA_TRANSIENT_STATUS_CODES:
        return True
...
def _get_crypto_bars(api: tradeapi.REST, ticker: str, start_date: str, end_date: str, raise_on_auth: bool = False):
    ...
    try:
        ...
        if raise_on_auth and is_alpaca_auth_error(e):
            raise
    ...
    try:
        ...
        for ticker in tickers:
            ...
            for attempt in range(max_attempts):
                try:
                    if is_crypto_ticker(ticker, config):
                        ...
                        end_time = datetime.now().strftime("%Y-%m-%d")
                        bars = _get_crypto_bars(api, ticker, start_time, end_time, raise_on_auth=True)
                    ...
                    if bar:
                        ...
                        if isinstance(bar_time, (int, float)):
                            ...
                            dt = datetime.fromtimestamp(bar_time.timestamp())
                        else:
                            dt = datetime.now()
                        data = {
                            'ticker': ticker,
                    ...
                    if is_alpaca_auth_error(e):
                        logger.error(f"Alpaca authorization failed while fetching latest data for {ticker}: {e}")
                        auth_failed = True
                        break
                    if is_transient_alpaca_error(e) and attempt < max_attempts - 1:
                        wait_seconds = 2 * (attempt + 1)
                        logger.warning(
                            f"Transient error fetching latest data for {ticker} "
                            f"(attempt {attempt + 1}/{max_attempts}), retrying in {wait_seconds}s: {e}"
                        )
            ...
            if auth_failed:
                break
        if auth_failed:
            logger.error("Latest data fetch aborted due to Alpaca authentication failure.")
            return pd.DataFrame()
        if not data_list:
...
async def fetch_news(config):
    ...
    # Map tickers to company names / search terms for better NewsAPI matching
    TICKER_NAMES = {
        'AAPL': ['Apple', 'AAPL'], 'NVDA': ['Nvidia', 'NVDA'], 'AMZN': ['Amazon', 'AMZN'],
        'META': ['Meta', 'Facebook', 'META'], 'GOOGL': ['Google', 'Alphabet', 'GOOGL'],
        'TSLA': ['Tesla', 'TSLA'], 'MSFT': ['Microsoft', 'MSFT'], 'JPM': ['JPMorgan', 'JPM'],
        'XLE': ['Energy ETF', 'XLE'], 'LMT': ['Lockheed Martin', 'LMT'],
        'UNH': ['UnitedHealth', 'UNH'], 'XOM': ['Exxon', 'XOM'], 'CVX': ['Chevron', 'CVX'],
        'QQQ': ['Nasdaq', 'QQQ'], 'SPY': ['S&P 500', 'SPY'], 'IWM': ['Russell 2000', 'IWM'],
        'AMD': ['AMD', 'Advanced Micro'], 'INTC': ['Intel', 'INTC'],
        'BTC/USD': ['Bitcoin', 'BTC'], 'ETH/USD': ['Ethereum', 'ETH'],
        'SOL/USD': ['Solana', 'SOL'], 'LINK/USD': ['Chainlink', 'LINK'],
        'AVAX/USD': ['Avalanche', 'AVAX'], 'ADA/USD': ['Cardano', 'ADA'],
    }

    all_tickers = config.get('tickers', []) + [t for t in config.get('crypto', {}).get('tickers', [])]

    # Build search query using company names (not just ticker symbols)
    search_terms = []
    ticker_keywords = {}  # keyword -> ticker mapping for matching
    for t in all_tickers:
        names = TICKER_NAMES.get(t, [t])
        search_terms.extend(names[:1])  # Use primary name for query
        for name in names:
            ticker_keywords[name.lower()] = t

    url = "https://newsapi.org/v2/everything"
    params = {
        "q": " OR ".join(search_terms),
        "apiKey": config['newsapi_token'],
        "language": "en",
        "pageSize": 100,  # More articles = better ticker coverage
        "sortBy": "publishedAt",
    }
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, params=params, timeout=15) as resp:
                ...
                for article in articles:
                    ...
                    # Match by company name / keyword, not just ticker symbol
                    for keyword, ticker in ticker_keywords.items():
                        if keyword in text:
                            article['ticker'] = ticker
                            break
                matched = [a for a in articles if 'ticker' in a]
                logger.info(f"NewsAPI: {len(matched)}/{len(articles)} articles matched to tickers")
                return matched
    except Exception as e:
            ...
            try:
                ...
                if 0 <= idx < len(prepared_items):
                    ...
                    out.append({
                        ...
                        'publishedAt': prepared_items[idx]['item'].get('publishedAt', ''),
    ...
    for idx, pi in enumerate(prepared_items, 1):
        try:
            ...
            out.append({
                ...
                'magnitude': magnitude,
                'recency_weight': recency_weight,
                'publishedAt': pi['item'].get('publishedAt', ''),
                'detail': msg
            })
        except Exception as e:
...
def _grok_chat(api_key, messages, timeout, model='grok-4', max_tokens=1000, temperature=0):
    """Call xAI Grok API directly via requests (no openai SDK needed)."""
    resp = requests.post(
...
# ---------------------------------------------------------------------------
# Grok bias: secondary AI opinion for confidence scoring
# ---------------------------------------------------------------------------
_grok_bias_cache: dict = {}  # {ticker: {direction, score, reasoning}}
_grok_bias_last_fetch: float = 0.0


def fetch_grok_bias(tickers: list, config: dict, cache_minutes: int = 30):
    """Fetch bullish/bearish/neutral bias from Grok for each ticker.

    Results are cached for *cache_minutes* to avoid redundant API calls.
    Populates the module-level _grok_bias_cache dict.
    """
    global _grok_bias_cache, _grok_bias_last_fetch
    import time as _time

    now = _time.time()
    if now - _grok_bias_last_fetch < cache_minutes * 60 and _grok_bias_cache:
        logger.debug("Using cached Grok bias (still fresh)")
        return

    api_key = config.get('grok_api_key')
    timeout = config.get('sentiment', {}).get('llm_timeout_seconds', 30)
    if not api_key:
        logger.debug("No Grok API key — skipping Grok bias")
        return

    # Build a single batch prompt for all tickers
    ticker_list = ', '.join(tickers)
    prompt = (
        f"You are a concise market analyst. For each ticker below, give a one-word bias "
        f"(bullish, bearish, or neutral) and a confidence score from -1.0 (very bearish) "
        f"to 1.0 (very bullish), plus a brief 5-word reason.\n\n"
        f"Tickers: {ticker_list}\n\n"
        f"Reply ONLY as JSON array, e.g.: "
        f'[{{"ticker":"AAPL","direction":"bullish","score":0.6,"reason":"strong earnings momentum"}}]'
    )

    try:
        raw = _grok_chat(api_key, [{'role': 'user', 'content': prompt}], timeout, max_tokens=600)
        ...
        if json_match:
            ...
            _grok_bias_cache = new_cache
            _grok_bias_last_fetch = now
            directions = {t: v['direction'] for t, v in new_cache.items()}
            logger.info(f"Grok bias fetched for {len(new_cache)} tickers: {directions}")
        else:
            logger.warning(f"Could not parse Grok bias JSON from: {raw[:200]}")
    except Exception as e:
        logger.warning(f"Grok bias fetch failed: {e}")
...
def _analyze_sentiment_grok(prepared_items, config, timeout, batch_mode, now):
    ...
    if batch_mode:
        return _batch_sentiment_grok(api_key, timeout, prepared_items, now)
    else:
        return _sequential_sentiment_grok(api_key, timeout, prepared_items, now)


def _batch_sentiment_grok(api_key, timeout, prepared_items, now):
...
def _sequential_sentiment_grok(api_key, timeout, prepared_items, now):
...
def initialize_trade_log() -> str:
    ...
    try:
        with open(log_filename, 'w', newline='') as csvfile:
            fieldnames = [
                'timestamp', 'ticker', 'type', 'price', 'quantity', 'status',
                'confidence', 'agreement', 'direction', 'stop_loss', 'take_profit',
            ]
...
def log_trade(trade_data: dict, log_filename: str):
    """Log individual trades into the CSV file. Captures confidence + signal context for later calibration."""
    try:
...
def fetch_current_positions(api: tradeapi.REST, retries: int = 3) -> list:
    """Fetch current positions from Alpaca API with retry on connection errors."""
    for attempt in range(retries):
        try:
            ...
            if is_alpaca_auth_error(e):
                logger.error(f"Alpaca authorization error fetching positions: {e}")
                return []
            if is_transient_alpaca_error(e):
                if attempt < retries - 1:
                    wait_seconds = 2 * (attempt + 1)
                    logger.warning(
                        f"Connection error fetching positions (attempt {attempt + 1}/{retries}), "
                        f"retrying in {wait_seconds}s: {e}"
                    )
                    time.sleep(wait_seconds)
                    continue
                logger.error(f"Error fetching current positions after {retries} attempts: {e}")
                return []
            if "timed out" in str(e).lower() or "timeout" in str(e).lower():
                logger.debug(f"Alpaca API timeout fetching positions: {e}")
            else:
...
# Cache for insider/congress data
_insider_scores_cache = {}
_insider_scores_cache_time = None


def fetch_insider_congress_data(tickers: List[str], config: dict, cache_minutes: int = 60) -> dict:
    """
    Fetch insider and congressional trading data for all tickers.
    Uses caching to avoid excessive API calls.
    """
    global _insider_scores_cache, _insider_scores_cache_time

    if not INSIDER_AVAILABLE:
        return {}

    insider_config = config.get('insider_congress', {})
    if not insider_config.get('enabled', False):
        return {}

    # Check cache
    if _insider_scores_cache_time and (datetime.now() - _insider_scores_cache_time).total_seconds() < cache_minutes * 60:
        missing = [t for t in tickers if t not in _insider_scores_cache]
        if not missing:
            logger.debug("Using cached insider/congress scores")
            return {t: _insider_scores_cache[t] for t in tickers if t in _insider_scores_cache}

    try:
        scores = get_insider_scores(tickers, config, cache_minutes)
        _insider_scores_cache.update(scores)
        _insider_scores_cache_time = datetime.now()
        return scores
    except Exception as e:
        logger.warning(f"Failed to fetch insider/congress data: {e}")
        return {}
...
def add_fundamental_features(df: pd.DataFrame, fundamentals: dict, config: Optional[dict] = None,
                              is_backtest: bool = False) -> pd.DataFrame:
    """
    Add FinViz fundamental features to the DataFrame.

    !!! KNOWN LOOKAHEAD BIAS IN BACKTESTS !!!
    FinViz returns CURRENT fundamentals (today's P/E, today's debt/equity).
    This function broadcasts those values across every historical row for the
    ticker, so a 2024-01-15 AAPL bar gets 2026-05-02 fundamentals as features.

    For live trading the broadcast is correct (only one row exists per ticker).
    For backtest/training, this is a real leak. Set
    config.backtest.disable_fundamentals: true (default) to skip in backtests.
    ...
    # Lookahead guard: caller-marked backtest context.
    if is_backtest and isinstance(config, dict):
        backtest_cfg = config.get('backtest', {}) or {}
        if backtest_cfg.get('disable_fundamentals', True):
            logger.warning(
                "Fundamental features skipped: caller marked is_backtest=True. "
                "Reason: today's FinViz values would leak into historical bars. "
                "Set backtest.disable_fundamentals=false to override (NOT recommended)."
            )
            return df
...
_vix_cache = {'value': 0.0, 'ts': 0.0}


def _fetch_vix_level() -> float:
    """Fetch the latest ^VIX close via yfinance, cached for 30 minutes.

    Returns 0.0 on any failure so callers degrade gracefully.
    """
    try:
        now = time.time()
        if _vix_cache['ts'] and (now - _vix_cache['ts']) < 30 * 60:
            return _vix_cache['value']
        import yfinance as yf
        hist = yf.Ticker('^VIX').history(period='5d')
        if hist is None or hist.empty:
            return _vix_cache['value'] if _vix_cache['ts'] else 0.0
        value = float(hist['Close'].dropna().iloc[-1])
        _vix_cache['value'] = value
        _vix_cache['ts'] = now
        return value
    except Exception as e:
        logger.warning(f"Failed to fetch VIX level: {e}")
        return 0.0


def engineer_features(df: pd.DataFrame, include_fundamentals: bool = True,
                       config: Optional[dict] = None, is_backtest: bool = False) -> pd.DataFrame:
    """Perform feature engineering on the dataset, combining indicators from both scripts.

    Pass `is_backtest=True` from training/backtest contexts to enable lookahead
    guards in add_fundamental_features. Live cycles use is_backtest=False so
    today's fundamentals are correctly applied to today's bar.
    """
    if df.empty:
    ...
    for ticker in df['ticker'].unique():
        ...
        # --- Strategy 3.12: Two Moving Averages crossover signal ---
        # +1 when MA10 > MA50 (golden cross / uptrend), -1 when MA10 < MA50
        df_ticker['MA_Crossover'] = (df_ticker['MA10'] > df_ticker['MA50']).astype(int) * 2 - 1

        # --- Strategy 4.4: Internal Bar Strength (IBS) mean-reversion ---
        # IBS = (Close - Low) / (High - Low), ranges 0-1
        # Low IBS (< 0.2) = closed near daily low = oversold/buy signal
        # High IBS (> 0.8) = closed near daily high = overbought/avoid
        hl_range = df_ticker['high'] - df_ticker['low']
        df_ticker['IBS'] = ((df_ticker['close'] - df_ticker['low']) / hl_range.replace(0, float('nan'))).fillna(0.5)
    ...
    if df_list:
        ...
        if not vix_data.empty:
            ...
            # Live cycles get the real VIX level; backtests keep 0 to avoid
            # broadcasting today's VIX across historical bars (lookahead leak)
            df['VIX'] = 0 if is_backtest else _fetch_vix_level()
        ...
        if include_fundamentals:
            try:
                ...
                if tickers:
                    ...
                    if fundamentals:
                        df = add_fundamental_features(df, fundamentals, config=config, is_backtest=is_backtest)
...
def add_sentiment_features(df: pd.DataFrame, config, is_backtest: bool = False) -> pd.DataFrame:
    """
    Add sentiment scores as features to the DataFrame.

    !!! KNOWN LOOKAHEAD BIAS IN BACKTESTS !!!
    This function fetches news at call-time and broadcasts a SINGLE per-ticker
    sentiment score across every historical row. In live trading that's fine
    (the only row is today). In backtests/training, every historical bar gets
    today's sentiment as a feature, which is forward-looking.

    For honest backtests, either:
      (a) set sentiment.enabled: false in config to neutralize the leak, or
      (b) refactor to store news with publishedAt timestamps and join only
          articles published before each bar's close (real fix).

    Set config.backtest.disable_sentiment_in_backtest: true to programmatically
    skip sentiment computation when called inside the backtest pipeline.
    """
    ...
    # Caller-marked backtest context. Live cycles broadcast sentiment to historical
    # bars only as ML context; the FEATURE we use is just today's value.
    # The leak we're fixing: training/backtest pipelines using today's sentiment as
    # a label-correlated feature on years of historical bars.
    if is_backtest:
        # Opt-in PIT path: use the local news_cache.jsonl built up by live cycles.
        # Only correct as far back as the cache covers — older bars get 0.0.
        if backtest_cfg.get('use_point_in_time_sentiment', False):
            try:
                from news_cache import pit_sentiment_for_bars, cache_stats
                stats = cache_stats()
                logger.info(f"PIT sentiment enabled. News cache stats: {stats}")
                df = pit_sentiment_for_bars(df, lookback_days=int(backtest_cfg.get('pit_lookback_days', 3)))
                if 'Sentiment_Momentum' not in df.columns:
                    df['Sentiment_Momentum'] = 0.0
                return df
            except Exception as e:
                logger.error(f"PIT sentiment failed, falling back to zero: {e}", exc_info=True)
                df['Sentiment_Score'] = 0.0
                df['Sentiment_Magnitude'] = 0.0
                df['Sentiment_Momentum'] = 0.0
                return df

        if backtest_cfg.get('disable_sentiment', True):
            df['Sentiment_Score'] = 0.0
            df['Sentiment_Magnitude'] = 0.0
            df['Sentiment_Momentum'] = 0.0
            logger.warning(
                "Sentiment computation skipped: caller marked is_backtest=True. "
                "Reason: live-fetched sentiment leaks future info into historical rows. "
                "Enable backtest.use_point_in_time_sentiment to use the local PIT cache instead."
            )
            return df
    ...
    # Append to local PIT cache so future backtests can use today's sentiment
    # as "knowable on $publishedAt". No-op if items lack publishedAt.
    try:
        from news_cache import append_to_cache
        n_cached = append_to_cache(sentiments)
        if n_cached:
            logger.info(f"Cached {n_cached} scored sentiment items to news_cache.jsonl for PIT backtests")
    except Exception as e:
        logger.warning(f"Could not write to news cache: {e}")
...
def generate_signals(model: XGBClassifier, data: pd.DataFrame, selected_features: List[str], config, q_table, threshold: float = 0.5, positions_snapshot: dict = None, persist: bool = True) -> pd.DataFrame:
    ...
    try:
        ...
        for ticker in config['tickers']:
            ...
            if last:
                ...
                # Use actual position P&L when available (realized outcome),
                # fall back to price delta only if no position data exists.
                actual_pnl_pct = None
                if position_info:
                    entry_price = position_info.get('avg_entry_price') or position_info.get('entry_price')
                    if entry_price and float(entry_price) > 0:
                        actual_pnl_pct = (current_price - float(entry_price)) / float(entry_price)

                if last_action == 'sell':
                    # Reward selling if price dropped after (avoided loss)
                    reward = -delta  # price went down = positive reward for selling
                elif last_action == 'buy':
                    # Use actual P&L from position if available
                    reward = actual_pnl_pct if actual_pnl_pct is not None else delta
                elif last_action == 'hold':
                    if last_holding_state == 1:
                        reward = actual_pnl_pct if actual_pnl_pct is not None else delta
                    else:
                        # Held cash — reward if price dropped (avoided loss)
                        reward = -delta if delta < 0 else 0.0
                else:
                    reward = 0.0
                alpha = 0.1
                gamma = 0.9
            ...
            action = select_action(q_table, state, holding_state, epsilon=0.0)

            # --- MA Crossover Override (Strategy 3.12) ---
            # When Q-table says HOLD but MA10 > MA50 (uptrend) and we don't
            # hold, promote to BUY so the confidence system can evaluate it.
            # When MA10 < MA50 (downtrend) and Q-table says BUY, demote to HOLD.
            ma_cross = ticker_data['MA_Crossover'].iloc[-1] if 'MA_Crossover' in ticker_data.columns else 0
            ibs_val = ticker_data['IBS'].iloc[-1] if 'IBS' in ticker_data.columns else 0.5
            if action == 'hold' and holding_state == 0 and ma_cross > 0:
                action = 'buy'
                logger.info(f"{ticker}: MA crossover override HOLD→BUY (MA10>MA50, IBS={ibs_val:.2f})")
            elif action == 'buy' and ma_cross < 0:
                action = 'hold'
                logger.info(f"{ticker}: MA crossover veto BUY→HOLD (MA10<MA50, downtrend)")

            # Override XGBoost signal with Q-learning / MA-adjusted action
            # (only on the LAST row for the ticker — not the whole history)
            ticker_last_idx = data.index[data['ticker'] == ticker][-1]
            if action == 'buy' and data.loc[ticker_last_idx, 'Signal'] != 1:
                data.loc[ticker_last_idx, 'Signal'] = 1
            elif action == 'sell' and data.loc[ticker_last_idx, 'Signal'] != -1:
                data.loc[ticker_last_idx, 'Signal'] = -1
...
def simulate_trades(data: pd.DataFrame, initial_capital: float, config, max_position: int = 100,
                    ...
                    slippage_pct: Optional[float] = None) -> Tuple[pd.DataFrame, List[Tuple]]:
    """Simulate trades based on generated signals.

    Cost assumptions (config.backtest.* overrides defaults):
      - slippage_pct: per-side slippage applied symmetrically to buys/sells.
        Default 0.0005 (5 bps), realistic for liquid retail orders. Round-trip = 10 bps.
      - transaction_cost_pct: per-side commission/fee. Default 0.001 (10 bps).
        Alpaca stocks are 0 commission, but crypto is ~15-25 bps; using a blended
        retail-realistic floor here so the backtest doesn't lie about edge.

    To explicitly model "no costs" for diagnostics, set both to 0.0 in config.
    """
    try:
        # Pull from config with realistic retail defaults; fall back to function args, then to defaults.
        backtest_cfg = (config or {}).get('backtest', {}) if isinstance(config, dict) else {}
        if slippage_pct is None:
            slippage_pct = float(backtest_cfg.get('slippage_pct', 0.0005))   # 5 bps per side
        if transaction_cost_pct is None:
            transaction_cost_pct = float(backtest_cfg.get('transaction_cost_pct', 0.001))  # 10 bps per side

        round_trip_bps = (slippage_pct + transaction_cost_pct) * 2 * 10000
        logger.info(
            f"Backtest cost model: slippage={slippage_pct*10000:.1f}bps/side, "
            f"transaction_cost={transaction_cost_pct*10000:.1f}bps/side, "
            f"round-trip drag={round_trip_bps:.0f}bps"
        )
...
def calculate_performance(capital_history: pd.DataFrame, drawdown_info: List[Tuple]):
    """Calculate and print performance metrics, persist summary as JSON for dashboard."""
    try:
        ...
        drawdown_series = (peak - capital_history['capital']) / peak.replace(0, np.nan)
        max_drawdown_pct = float(drawdown_series.max() or 0) * 100
        max_drawdown_abs = (capital_history['capital'].cummax() - capital_history['capital']).max()

        positive_days = (capital_history['Returns'] > 0).sum()
        active_days = (capital_history['Returns'] != 0).sum()
        win_rate = (positive_days / active_days * 100) if active_days > 0 else 0.0

        logger.info(f"Cumulative Returns: {cumulative_returns:.2%}")
        logger.info(f"Sharpe Ratio: {sharpe_ratio:.2f}")
        logger.info(f"Win Rate: {win_rate:.1f}%")
        logger.info(f"Maximum Drawdown: {max_drawdown_abs:.2f} ({max_drawdown_pct:.1f}%)")
        ...
        try:
            results = {
                "total_return_pct": float(cumulative_returns) * 100,
                "sharpe_ratio": float(sharpe_ratio),
                "win_rate": float(win_rate),
                "max_drawdown_pct": float(max_drawdown_pct),
                "max_drawdown_abs": float(max_drawdown_abs),
                "starting_capital": float(capital_history['capital'].iloc[0]),
                "ending_capital": float(capital_history['capital'].iloc[-1]),
            ...
            logger.info(f"Backtest results JSON saved to {ARTIFACT_DIR}/backtest_results.json")
        except Exception as save_err:
            logger.error(f"Failed to save backtest_results.json: {save_err}")
    except Exception as e:
...
def send_trade_notification(ticker: str, action: str, price: float, quantity: float, order_type: str, error: str = None):
    ...
    try:
        ...
        # (use globals() lookup — the module-level `config` only exists when legacy main() runs)
...
def backtest_strategy(model: XGBClassifier, data: pd.DataFrame, selected_features: List[str], config, q_table, initial_capital: float = 10000):
    ...
    data = generate_signals(model, data, selected_features, config, q_table, threshold=0.5, positions_snapshot={}, persist=False)
...
def place_order(api: tradeapi.REST, symbol: str, qty: float, side: str, order_type: str = 'market', time_in_force: str = 'day', current_price: float = None, use_smart_order: bool = True, config: dict = None):
    """
    Place an order through Alpaca API with smart execution.

    Smart order logic:
    1. Try limit order at better price (configurable improvement %)
    ...
    Args:
        ...
        config: Config dict for smart order settings
    """
    try:
        # Smart order: Try limit first, fallback to market
        if use_smart_order and current_price and order_type in ['market', 'smart']:
            try:
                # Calculate limit price (configurable % better than market, default 0.3%)
                smart_cfg = (config or {}).get('smart_orders', {})
                improvement_pct = smart_cfg.get('limit_improvement_pct', 0.3) / 100
                if side == 'buy':
                    limit_price = round(current_price * (1 - improvement_pct), 2)
                else:
                    limit_price = round(current_price * (1 + improvement_pct), 2)
...
def kelly_fraction(win_prob: float, win_pct: float, loss_pct: float) -> float:
    """Compute Kelly criterion: f = p/a - q/b where p=win_prob, q=1-p, a=loss_pct, b=win_pct.

    Returns the fraction of equity to risk on a single trade. Negative means
    no edge (don't bet). Caller is responsible for applying fractional Kelly,
    minimum-edge thresholds, and any hard caps.
    """
    if win_pct <= 0 or loss_pct <= 0:
        return 0.0
    p = max(0.0, min(1.0, float(win_prob)))
    q = 1.0 - p
    return (p / float(loss_pct)) - (q / float(win_pct))

def execute_trading_logic_live(api: tradeapi.REST, data: pd.DataFrame, config, q_table, guard: DayTradeGuard, dexter_gate=None, buying_power_pct: float = 100):
    """Execute trading logic using hybrid signals during live trading with PDT guard."""
    if data.empty:
        logger.warning("No data available for executing trading logic. Skipping trading.")
        return

    # Safety check: Don't trade if too close to market close.
    # Configurable via config.schedule.no_new_entries_after (default 15:45 ET)
    now_et = datetime.now(tz=pytz.timezone('US/Eastern'))
    today_et = now_et.date()
    try:
        _cutoff_str = (config or {}).get('schedule', {}).get('no_new_entries_after', '15:45')
        _ch, _cm = map(int, str(_cutoff_str).split(':'))
        cutoff_time = dt_time(_ch, _cm)
    except Exception:
        cutoff_time = dt_time(15, 45)  # 3:45 PM ET fallback
    ...
    # --- Crypto allocation cap: how much of equity can sit in crypto (combined) ---
    crypto_cfg = config.get('crypto', {}) or {}
    crypto_alloc_cap_pct = float(crypto_cfg.get('allocation_cap_pct', 100.0))
    equity = float(getattr(account, 'equity', available_cash))
    crypto_max_value = equity * (crypto_alloc_cap_pct / 100.0)
    current_crypto_value = sum(
        pos['quantity'] * pos['current_price']
        for pos in current_positions
        if is_crypto_ticker(pos['ticker'], config)
    )
    crypto_room = max(crypto_max_value - current_crypto_value, 0.0)
    if crypto_alloc_cap_pct < 100.0:
        logger.info(
            f"Crypto allocation: ${current_crypto_value:,.2f} / ${crypto_max_value:,.2f} "
            f"({crypto_alloc_cap_pct:.0f}% cap of ${equity:,.2f} equity). "
            f"Room for new crypto: ${crypto_room:,.2f}"
        )

    # Fetch Grok bias (secondary AI) for all tickers — cached, won't re-fetch within 30 min
    all_tickers = list(data['ticker'].unique())
    try:
        fetch_grok_bias(all_tickers, config)
    except Exception as e:
        logger.warning(f"Grok bias fetch failed (non-fatal): {e}")

    trade_count = 0
    successful_trades = 0

    # --- Multi-Asset Trend Following (Strategy 4.6): Concentrate into top signals ---
    max_trades = config.get('max_concurrent_trades', 5)
    ticker_scores = {}
    for t in data['ticker'].unique():
        t_rows = data[data['ticker'] == t]
        if t_rows.empty:
            continue
        t_row = t_rows.iloc[-1]
        sig = t_row.get('Signal', 0)
        ma_cross = t_row.get('MA_Crossover', 0)
        ibs_val = t_row.get('IBS', 0.5)
        # Score: signal direction aligned with MA trend, bonus for oversold IBS
        score = abs(sig) * (1.0 if sig * ma_cross > 0 else 0.3)
        if sig > 0 and ibs_val < 0.3:  # buying + oversold = strong
            score *= 1.5
        ticker_scores[t] = score

    ranked = sorted(ticker_scores.items(), key=lambda x: x[1], reverse=True)
    top_tickers = set(t for t, s in ranked[:max_trades])
    # Always include tickers we already hold (for sell/stop-loss signals)
    for pos in positions_dict:
        top_tickers.add(pos)
    logger.info(f"Ticker ranking: {[(t, f'{s:.2f}') for t, s in ranked]}. Trading top {max_trades}: {sorted(top_tickers)}")

    for idx, row in data.iterrows():
        ticker = row['ticker']
        signal = row['Signal']

        # Skip tickers not in top-ranked set (concentration strategy)
        ...
        # Guard against stale daily stock bars (common when forcing runs before market data is available).
        if not is_crypto:
            try:
                signal_date = datetime.strptime(str(date)[:10], '%Y-%m-%d').date()
            except Exception:
                signal_date = None
            if signal_date and signal_date < today_et:
                logger.info(f"Skipping stale stock signal for {ticker}: signal date {signal_date} (today {today_et}).")
                continue
        ...
        # Crypto requires USD cash (no margin); stocks can use buying power
        funds_available = available_cash if is_crypto else buying_power
        position_size = min((risk_per_trade / atr), (funds_available / current_price))
        if max_position_value > 0:
            position_size = min(position_size, (max_position_value / current_price))
        # Fractional shares: round to 2 decimals for stocks, keep full precision for crypto
        if not is_crypto:
            position_size = round(position_size, 2)
        # Minimum trade value check instead of integer share check
        min_trade_value = float(config.get('min_trade_value', 10.0))
        trade_value = position_size * current_price
        if trade_value < min_trade_value:
            logger.info(f"Trade value ${trade_value:.2f} below ${min_trade_value:.0f} minimum for {ticker} on {date}. Skipping.")
            continue
        ...
        confidence = None  # Default; populated below when enabled
        confidence_cfg = config.get('confidence', {})
        if confidence_cfg.get('enabled', True) and signal != 0:
            # Get Grok bias for this ticker (secondary AI opinion)
            grok_bias = _grok_bias_cache.get(ticker)

            # Get sentiment score from row (primary AI: Claude)
            ...
            # Get FinViz risk from fundamentals
            finviz_risk = None

            # Get insider/congress sentiment
            insider_data = None
            if INSIDER_AVAILABLE:
                insider_scores = fetch_insider_congress_data(
                    config.get('tickers', []), config
                )
                insider_data = insider_scores.get(ticker)

            # Get MA crossover and IBS signals from the row
            ma_crossover_val = row.get('MA_Crossover', None)
            if ma_crossover_val is not None:
                ma_crossover_val = int(ma_crossover_val)
            ibs_val = row.get('IBS', None)
            if ibs_val is not None:
                ibs_val = float(ibs_val)
            ...
            # Options flow (9th source). None for crypto / unavailable.
            options_flow_data = None
            try:
                from options_flow import get_options_flow
                options_flow_data = get_options_flow(ticker, config)
            except Exception as e:
                logger.debug(f"[{ticker}] options_flow lookup skipped: {e}")
            ...
            # --- Position sizing: fixed-fractional with confidence multiplier ---
            # NOTE: Kelly sizing was reverted on 2026-05-02 because the confidence score is a
            # weighted vote across heterogeneous signals, not a calibrated probability of winning.
            # Plugging it into Kelly produces systematically misszied bets. Once we have ~200+
            # trades + logged confidence values, we can calibrate confidence -> P(win) via Platt
            # scaling/isotonic regression and reintroduce Kelly with real probabilities.
            ...
            trade_value = position_size * current_price
            if trade_value < min_trade_value:
                logger.info(f"Trade value ${trade_value:.2f} below ${min_trade_value:.0f} minimum for {ticker} after confidence scaling. Skipping.")
                continue

        if trade_value > (available_cash if is_crypto else buying_power):
            logger.info(f"Insufficient {'cash' if is_crypto else 'buying power'} to execute trade for {ticker} on {date}.")
            continue

        # Crypto allocation cap: refuse new crypto buys (signal==1) that would push us over the cap.
        # Sells (signal==-1) and stock trades are unaffected.
        if is_crypto and signal == 1 and crypto_alloc_cap_pct < 100.0:
            if trade_value > crypto_room:
                logger.info(
                    f"Crypto allocation cap reached for {ticker}: trade ${trade_value:.2f} "
                    f"exceeds remaining room ${crypto_room:.2f} "
                    f"(current crypto ${current_crypto_value:.2f} / cap ${crypto_max_value:.2f}). Skipping."
                )
                continue
        ...
        if signal == 1:
            if ticker not in positions_dict or positions_dict[ticker]['quantity'] == 0:
                # Dexter gate veto: only blocks new BUY entries, never sells/exits
                if dexter_gate is not None and hasattr(dexter_gate, 'should_allow') and not dexter_gate.should_allow(ticker):
                    logger.info(f"DEXTER GATE: skipping {ticker}")
                    continue
                qty = round(float(position_size), 2) if not is_crypto else float(position_size)
                min_qty = float(asset_config.get('min_qty', 0.0001)) if is_crypto else 0.01
                if qty < min_qty:
                ...
                is_fractional = not is_crypto and qty != int(qty)
                scaled_exits_enabled = config.get('scaled_exits', {}).get('enabled', False)
                use_bracket = config.get('use_bracket_orders', True) and not is_crypto and not is_fractional

                if use_bracket and not scaled_exits_enabled:
                    try:
                        ...
                        order = place_order(api, ticker, qty, 'buy', time_in_force=time_in_force, current_price=current_price, config=config)
                elif use_bracket and scaled_exits_enabled:
                    try:
                        # Stop-loss only OTO order — profit taking handled by scaled exits in position_monitor
                        logger.info(f"Placing stop-loss order for {ticker}: Entry={current_price:.2f}, "
                                   f"Stop={stop_loss_price:.2f} ({dynamic_stop_loss_pct:.1%}) [scaled exits handle TP]")
                        order = api.submit_order(
                            symbol=ticker,
                            qty=qty,
                            side='buy',
                        ...
                        logger.info(f"OTO stop-loss order submitted for {ticker}: {order.id}")
                    except Exception as oto_error:
                        logger.warning(f"OTO order failed for {ticker}: {oto_error}. Falling back to simple order.")
                        order = place_order(api, ticker, qty, 'buy', time_in_force=time_in_force, current_price=current_price, config=config)
                else:
                    order = place_order(api, ticker, qty, 'buy', time_in_force=time_in_force, current_price=current_price, config=config)
                ...
                if order:
                    ...
                    if is_crypto:
                        current_crypto_value += trade_value
                        crypto_room = max(crypto_max_value - current_crypto_value, 0.0)
                    trade_count += 1
                    log_trade({
                        'ticker': ticker,
                        ...
                        'status': 'submitted',
                        'confidence': f"{confidence.score:.4f}" if confidence is not None else '',
                        'agreement': f"{confidence.agreement_pct:.4f}" if (confidence is not None and hasattr(confidence, 'agreement_pct')) else '',
                        'direction': getattr(confidence, 'direction', '') if confidence is not None else '',
                    }, log_filename)
            ...
            if ticker in positions_dict and positions_dict[ticker]['quantity'] > 0:
                # PDT guard: block same-day sells for stocks if no day trades remaining
                entry_date_str = guard.current_open_entry_date(ticker) if not is_crypto else ""
                today_str = now_dt.strftime("%Y-%m-%d")
                if not is_crypto and entry_date_str == today_str and guard_remaining <= 0:
                    logger.info(f"PDT guard blocking same-day sell for {ticker}; would create day trade (remaining: {guard_remaining})")
                    continue
                qty = round(float(position_size), 2) if not is_crypto else float(position_size)
                quantity_to_sell = min(positions_dict[ticker]['quantity'], qty)
                trade_value = quantity_to_sell * current_price
                order = place_order(api, ticker, quantity_to_sell, 'sell', time_in_force=time_in_force, current_price=current_price, config=config)
                if order:
                    buying_power += trade_value
                    available_cash += trade_value
                    if is_crypto:
                        current_crypto_value = max(current_crypto_value - trade_value, 0.0)
                        crypto_room = max(crypto_max_value - current_crypto_value, 0.0)
                    trade_count += 1
                    log_trade({
                        'ticker': ticker,
        ...
        with job_lock:
            ...
            if execution_start <= now_time <= execution_end and current_time.weekday() < 5:
                ...
                if not latest_data.empty:
                    ...
                    engineered_data = engineer_features(raw_historical, config=config)
...
def main():
    ...
    # PDT regulatory cutover gate: FINRA Rule 4210 effective 2026-06-04. See bot_service.py.
    ...
    effective_dt_enabled = cfg_dt_enabled if _date.today() >= PDT_RETIRES_ON else False
    if _date.today() < PDT_RETIRES_ON and cfg_dt_enabled:
        logger.warning(f"day_trading.enabled=true forced to False until PDT retires on {PDT_RETIRES_ON}.")
    guard = DayTradeGuard(
        max_day_trades=day_trading_cfg.get('max_day_trades', config.get('max_day_trades', 2)),
